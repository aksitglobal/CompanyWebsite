...
@section('title', 'Class Schedule Management — Admin')
...
<div class="container my-5" style="min-height: 70vh;">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2><i class="fas fa-calendar-alt text-info me-2"></i> Class Schedule Management</h2>
        ...
        <div class="col-lg-5">
            <div class="card shadow-sm h-100">
                <div class="card-header bg-dark text-white">
                    <h5 class="mb-0"><i class="fas fa-upload me-2"></i>Upload New Class Schedule PDF</h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('admin.class-schedule.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                ...
                <div class="card-body p-0">
                    @if($current)
                            ...
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <div>
                                    <div class="fw-semibold text-truncate" style="max-width: 320px;">
                                        {{ basename($current->pdf_path) }}
                                    </div>
                                    <small class="text-muted">
                                        Uploaded: {{ $current->uploaded_at->format('d M Y, h:i A') }}
                                    </small>
                                </div>
                            </div>
                        </div>
                        <iframe
                            ...
                            title="Current Class Schedule PDF"
                        ></iframe>
                        <div class="p-3 border-top bg-light d-flex gap-2 flex-wrap">
                            <a href="{{ $pdfUrl }}" download class="btn btn-sm btn-primary">
                                <i class="fas fa-download me-1"></i>Download
                            </a>
                            <a href="{{ route('class-schedule') }}" target="_blank" class="btn btn-sm btn-outline-secondary">
                                <i class="fas fa-globe me-1"></i>View Public Page
                            </a>
                            <form action="{{ route('admin.class-schedule.destroy') }}" method="POST" class="m-0"
                                  onsubmit="return confirm('Are you sure you want to delete this PDF? This cannot be undone.')">
                                @csrf
                                @method('DELETE')
                    ...
                    @else
                        <div class="d-flex flex-column align-items-center justify-content-center py-5 text-center text-muted">
                            ...
                            <p class="mb-0">Upload a PDF using the form on the left to display it on the public class schedule page.</p>
                        </div>
                    @endif
                </div>
            </div>
